...
const postOneFile = async (req, res, next) => {
  ...
  const user= await User.findById(req.user.id,{password:0,email:0,__v:0,createdAt:0,updatedAt:0})
  if(!user){
...
const getCurrUserPosts = async (req,res,next)=>{
  ...
  const reels= await Post.find({username:user.username,isReel:true}).sort({createdAt:-1})
  res.status(200).json({message:"Posts fetched successfully",posts,reels})
}

const getUserPosts=async (req,res,next)=>{
  const posts=await Post.find({username:req.params.username}).sort({createdAt:-1})
  const reels= await Post.find({username:req.params.username,isReel:true}).sort({createdAt:-1})
...
const likePost=async (req,res,next)=>{
  ...
  const commentPost=async (req,res,next)=>{
    ...
    const newComment=new Comment({user:req.user.id,post:req.params.id,comment:commnet})
    ...
    const deleteComment=async (req,res,next)=>{
      ...
      const likesCount=async (req,res,next)=>{
        ...
        res.status(200).json({message:"Likes fetched successfully",likes:likes.length})
        }
        ...
        const commentsCount=async (req,res,next)=>{
          ...
          res.status(200).json({message:"Comments fetched successfully",comments:comments.length})
          }


export { postOneFile,getCurrUserPosts,getUserPosts,deletePost,likePost,commentPost,deleteComment,likesCount,commentsCount };