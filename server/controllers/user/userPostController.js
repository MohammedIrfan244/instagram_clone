...
const likePost = async (req, res, next) => {
  ...
  await Post.findByIdAndUpdate(req.params.id, { $inc: { likesCount: 1 } });
  await newLike.save();
  const receiver = await User.findOne({ username: post.username });
  ...
  if (userSocketMap[receiver._id]) {
    const currUser = await User.findById(req.user.id);
    io.to(userSocketMap[receiver._id]).emit("newNotification", {...notification._doc ,sender:{username:currUser.username,profile:currUser.profile,fullname:currUser.fullname}});
  }
...
const savePost = async (req, res, next) => {
  ...
  if (saved) {
    ...
    return res
      .status(200)
      .json({ message: "Post unsaved successfully", isSaved: false });
  }
